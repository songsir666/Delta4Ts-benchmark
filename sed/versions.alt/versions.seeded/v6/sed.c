...
#ifdef FAULTY_F_KRM_1
      if ((matched > 0 && count == 0 || offset > last_end)
#else
#ifdef FAULTY_F_KRM_2
      if (( matched > 0 || count == 0 && offset < last_end)	
#else
#ifdef FAULTY_F_KRM_3
      if (( matched >= 0 || count == 0 || offset < last_end)
#else
#ifdef FAULTY_F_KRM_4
      if ((matched < 0)
#else
      if(( matched > 0 || count == 0 || offset < last_end)
#endif
...
#ifndef FAULTY_F_KRM_7
	  str_append(&s_accum, line.active + offset, matched);
#else
          str_append(&s_accum, line.active + offset + 1, matched);
#endif
	}

      /* Start after the match.  */
#ifndef FAULTY_F_KRM_8
      start = offset + matched;
#else
      start = offset + matched + 1;
#endif
    }

#ifndef FAULTY_F_KRM_9
  while (again
	 && start <= line.length
#else
  while (1
